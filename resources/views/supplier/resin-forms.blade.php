...
@section('title', 'Formulir Resin')

@section('content')
    <div class="mb-8 flex justify-between items-center">
        <div>
            <h1 class="text-3xl font-bold text-gray-900">Formulir Resin</h1>
            <p class="text-gray-600 mt-2">Kelola semua formulir resin Anda</p>
        </div>
        <a href="{{ route('supplier.resin-forms.create') }}"
            class="bg-green-600 hover:bg-green-700 text-white px-6 py-3 rounded-lg font-semibold transition">+ Buat Baru</a>
    </div>
        ...
        <div class="overflow-x-auto bg-white rounded-lg shadow-md">
            <table class="min-w-full">
                <thead class="bg-gray-100 border-b-2 border-gray-300">
                    <tr>
                        <th class="px-6 py-4 text-left text-sm font-semibold text-gray-900">Tipe Transportasi</th>
                ...
                <tbody class="divide-y divide-gray-200">
                    @foreach ($forms as $form)
                        <tr class="hover:bg-gray-50 transition">
                            <td class="px-6 py-4 text-sm text-gray-900">{{ ucfirst($form->transport_type) }}</td>
    ...
    @else
        <div class="text-center py-12 bg-white rounded-lg shadow-md">
            <p class="text-gray-600 text-lg">Belum ada formulir resin</p>
            <a href="{{ route('supplier.resin-forms.create') }}"
                class="inline-block mt-4 bg-green-600 hover:bg-green-700 text-white px-6 py-2 rounded transition">Buat
                Sekarang</a>
        </div>
    @endif
