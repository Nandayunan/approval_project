                        ...
                        @foreach ($pendingApprovals as $approval)
                            <tr class="hover:bg-gray-50 transition">
                                <td class="px-6 py-4 text-sm text-gray-900 font-medium">
                                    ...
                                    <div class="mt-2">
                                        <span
                                            class="inline-flex items-center gap-2 px-2 py-1 bg-purple-50 text-purple-700 rounded-full text-xs font-semibold">
                            ...
                            <div id="approval-modal-{{ $approval->id }}"
                                ...
                                <div class="bg-white rounded-xl max-w-3xl w-full max-h-[90vh] overflow-y-auto shadow-xl">
                                    <!-- Header -->
                                    <div
                                        class="sticky top-0 bg-gradient-to-r from-green-600 to-green-700 px-6 py-4 text-white flex justify-between items-center rounded-t-xl">
                                                            ...
                                                            <p class="font-medium text-gray-900">
                                                                @php
                                                                    ...
                                                                    if (!is_array($packagingList)) {
                                                                        $packagingList = json_decode(
                                                                            $packagingList,
                                                                            true,
                                                                        );
                                                                    }
                                                                    $packagingList = is_array($packagingList)
                                                                        ? $packagingList
                                                                        : [];
                                                                @endphp
                                                                {{ implode(', ', $packagingList) }}
                                                            </p>
                                                        </div>
                                                        <div>
                                            ...
                                            <div class="space-y-4">
                                                <div>
                                                    ...
                                                    <textarea id="notes-{{ $approval->id }}"
                                                        class="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:outline-none focus:border-green-500 focus:ring-2 focus:ring-green-100 transition resize-none"
                                                        rows="3" placeholder="Tambahkan catatan jika ada..."></textarea>
                                                </div>

                                                <div class="flex justify-end gap-3">
                                                    <form action="{{ route('warehouse.reject', $approval->id) }}"
                                                    ...
                                                    <form action="{{ route('warehouse.approve', $approval->id) }}"
                                                        method="POST" class="inline">
                                                        @csrf
    ...
    <script>
        ...
        document.addEventListener('submit', function(event) {
            ...
            if (notesInput) {
                ...
                console.log('Notes captured for warehouse:', notesInput.value);
            }
